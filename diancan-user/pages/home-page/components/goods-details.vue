		...
		<view class="details-padd">
			...
			<view class="describe">
				...
				<view class="details-quantity" v-if="new_data.res_data.itemgood.in_stock > 0">
					<view><image v-if="new_data.res_data.itemgood.quantity > 0" src="/static/tab/jianhao.png" @click="reduce(new_data.res_data,new_data.res_index)"></image></view>
					<view><text v-if="new_data.res_data.itemgood.quantity > 0">{{new_data.res_data.itemgood.quantity}}</text></view>
					<view><image src="/static/tab/jia.png" @click="plus(new_data.res_data,new_data.res_index)"></image></view>
				</view>
				<!-- 已售罄 -->
...
export default{
	...
	methods:{
		...
		// -
		reduce(pro_details,res_index){
			this.pubLic(pro_details,res_index,'RE')
		},
		// +
		plus(pro_details,res_index){
			this.pubLic(pro_details,res_index,'PL')
		},
		// 加减公用
		pubLic(pro_details,res_index,value){
			...
			let specs_id = this.specs_id == '' ? itemgood.specs[res_index].specs_id : this.specs_id
			if(value == 'RE'){
